...
class ExpenseReportTest(TestCase):
    ...
    def setUp(self):
        ...
        self.e1a = Expense(
            description="e1a", amount=10.52, date=date(2013, 1, 31), category=self.cat1)
        self.e1a.save()
        self.e1b = Expense(
            description="e1b", amount=1.10, date=date(2013, 1, 1), category=self.cat1)
        self.e1b.save()
        self.e2a = Expense(
            description="e2a", amount=1000, date=date(2013, 3, 31), category=self.cat2)
        self.e2a.save()
    ...
    def test_expense_report_totals(self):
        ...
        self.assertEqual(report['total_per_month'], float(
            report['total']) / float(report['months']))
